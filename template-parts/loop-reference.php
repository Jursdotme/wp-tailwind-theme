...
$taxonomy_terms = get_the_terms($post->ID, 'reference_filter'); // change to your taxonomy
$classes = [];
if ($taxonomy_terms) {
  foreach ($taxonomy_terms as $taxonomy_term) {
    $classes[] = $taxonomy_term->slug;
  }
}
...
<article id="post-<?php the_ID(); ?>" <?php post_class('flex flex-col overflow-hidden rounded-lg shadow-lg '); ?> x-show.transition="visibilityCheck(['<?php echo implode("','", $classes); ?>'])">
